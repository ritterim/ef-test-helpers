...
{
    public class MockableDbSetTests
    {
        [Fact]
...
#pragma warning restore 612, 618
        ...
        [Fact]
        public async Task Moq_DbSet_can_be_used_for_getting_async_results()
        {
            var products = new[] { new Product { Id = 1 }, new Product { Id = 2 } };
            var mockSet = CreateMockQueryableSet(new InMemoryAsyncQueryable<Product>(products.AsQueryable()));

            DbSet<Product> dbSet = mockSet.Object;
            Assert.Equal(new[] { 1, 2 }, (await dbSet.ToListAsync()).Select(p => p.Id));
        ...
        public async Task Moq_DbSet_can_be_used_for_async_SqlQuery()
        {
            ...
            Assert.Equal(new[] { 1, 2 }, (await mockSet.Object.SqlQuery(query, parameters).ToListAsync()).Select(p => p.Id));
            mockSet.Verify(m => m.SqlQuery(query, parameters), Times.Once());
        }
